...
<div class="form-group row {{ $errors->has('is_default') ? 'has-error' : '' }}">
    ...
    <div class="col-sm-10">
      ...
      @if(isset($typemouvement->id) && $typemouvement->is_default)
...
<div class="form-group row {{ $errors->has('statut_id') ? 'has-error' : '' }}">
    ...
    <div class="col-sm-10">
      <input name="statut_id" {{ Auth::user()->can(\App\TypeMouvement::canchange_statut()) ? '' : 'disabled' }} class="toggle-class" type="checkbox" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-on="Actif" data-off="Inactif" data-size="xs" {{ $typemouvement->statut->code == 'actif' ? 'checked' : '' }}>
