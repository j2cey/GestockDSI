...
<div class="form-group row {{ $errors->has('statut_id') ? 'has-error' : '' }}">
    ...
    <div class="col-sm-10">
        <input name="statut_id" {{ Auth::user()->can(\App\SockLieu::canchange_statut()) ? '' : 'disabled' }} class="toggle-class" type="checkbox" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-on="Actif" data-off="Inactif" data-size="xs" {{ $stocklieu->statut->code == 'actif' ? 'checked' : '' }}>
