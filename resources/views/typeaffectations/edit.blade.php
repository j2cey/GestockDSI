...
@section('breadcrumb')
  {{ Breadcrumbs::render('typeaffectations.edit',$typeaffectation->id) }}
@endsection
    ...
    <div class="card m-b-30">
      <div class="card-body">
        <h4 class="mt-0 header-title">Modification</h4>
          <p class="text-muted m-b-30 font-14">Modification du Type d Affectation <code class="highlighter-rouge">{{ $typeaffectation->libelle }}</code>.</p>

          <form action="{{ route('typeaffectations.update', ['typeaffectation' => $typeaffectation]) }}" method="POST">
            @method('PUT')
